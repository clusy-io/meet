...
export function BookingPagesView({
  ...
  const readyCount = data.pages.filter(
    ...
    <section aria-labelledby="booking-pages-heading">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-ink-faint">
            <Globe2 className="h-3.5 w-3.5" strokeWidth={1.7} />
            Public booking experience
          </div>
          <h2
            id="booking-pages-heading"
            className="mt-2 font-serif-display text-3xl tracking-tight text-ink"
          >
            Booking pages
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-mute">
            Give every host their own voice, availability, invite details, and
            notification route.
          </p>
        </div>
        <div className="inline-flex w-fit items-center gap-2 rounded-full border border-hairline bg-paper-raise px-3 py-1.5 text-xs text-ink-mute">
          <Clock3 className="h-3.5 w-3.5" strokeWidth={1.6} />
          Team defaults · {data.hostTimezone.replaceAll("_", " ")}
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-3 gap-2 sm:gap-3">
        <SummaryCard
          ...
          detail="One for every host"
          icon={<UserRound />}
        />
        <SummaryCard
          label="Live now"
          value={liveCount}
          detail={`${readyCount} calendar-ready`}
          icon={<Eye />}
        />
        <SummaryCard
          ...
          detail={
            attentionCount === 0
              ...
              : "Live without a calendar"
          }
          icon={attentionCount === 0 ? <CheckCircle2 /> : <CircleAlert />}
          warning={attentionCount > 0}
        />
      </div>

      <div className="mt-6 grid items-start gap-6 lg:grid-cols-[minmax(15rem,0.72fr)_minmax(0,2.2fr)]">
        <aside
          className="rounded-2xl border border-hairline bg-paper-raise p-3 lg:sticky lg:top-32"
        ...
        >
          <div className="flex items-center justify-between px-2 pb-3 pt-1">
            ...
            <span className="rounded-full bg-ink/[0.05] px-2 py-1 font-mono text-[11px] text-ink-mute">
              {data.pages.length}
            </span>
          </div>
          <div className="space-y-2">
            {data.pages.map((page) => (
              <PageListCard
                ...
                livePending={livePendingKey !== null || editorMutationPending}
                selectionDisabled={editorMutationPending || livePendingKey !== null}
                error={pageErrors[page.memberKey]}
                copied={copiedKey === page.memberKey}
                onSelect={() => selectPage(page.memberKey)}
                onToggleLive={(enabled) => void toggleLive(page, enabled)}
...
function SummaryCard({
  ...
  return (
    <div className="min-w-0 rounded-xl border border-hairline bg-paper-raise p-3 sm:p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-ink-mute">{label}</p>
          <p className="mt-1 font-serif-display text-3xl leading-none text-ink">
            {value}
          </p>
        </div>
        <span
          className={`hidden rounded-lg border border-hairline p-2 sm:inline-flex ${warning ? "text-status-warn" : "text-ink-mute"}`}
        >
          <span className="block [&>svg]:h-4 [&>svg]:w-4 [&>svg]:stroke-[1.6]">
            {icon}
          </span>
        </span>
      </div>
      <p
        className={`mt-3 hidden text-xs sm:block ${warning ? "text-status-warn" : "text-ink-faint"}`}
      >
        {detail}
      </p>
    </div>
  ...
  return (
    <article
      className={`rounded-xl border p-3 transition-colors ${
        selected
          ? "border-hairline-strong bg-paper shadow-[0_1px_2px_hsl(var(--ink)_/_0.04)]"
          : "border-transparent bg-paper-raise hover:border-hairline hover:bg-paper"
      }`}
    >
      <div className="flex items-start gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-hairline bg-paper font-serif-display text-sm text-ink">
      ...
      <div className="mt-3 flex items-center gap-1.5 rounded-lg border border-hairline bg-paper-raise px-2.5 py-2">
        <Link2
      ...
      <button
        ...
        className={`${FOCUS_RING} mt-2 flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-xs font-medium transition-colors disabled:cursor-wait disabled:opacity-50 ${selected ? "text-ink" : "text-ink-mute hover:bg-ink/[0.04] hover:text-ink"}`}
      >
...
function BookingPageEditor({
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-hairline bg-paper-raise">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                className={`rounded-full border border-hairline px-2 py-0.5 text-[11px] font-medium ${page.enabled ? "text-status-ok" : "text-ink-faint"}`}
              >
                {page.enabled ? "Live" : "Paused"}
              </span>
              {!page.calendarReady && (
                <span className="rounded-full border border-hairline px-2 py-0.5 text-[11px] font-medium text-status-warn">
                  No ready calendar
                </span>
              )}
            </div>
          ...
          <div className="flex items-center justify-between gap-3 rounded-xl border border-hairline bg-paper px-3 py-2 sm:justify-start">
            <div>
      ...
      <div className="grid items-start gap-0 xl:grid-cols-[minmax(0,1fr)_17rem]">
        <form
        ...
        >
          <fieldset
            ...
            className="min-w-0 border-0 p-0 disabled:cursor-wait"
          >
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-hairline bg-paper px-3.5 py-3">
            <div className="flex items-center gap-2">
              <Settings2 className="h-4 w-4 text-ink-mute" strokeWidth={1.6} />
              <div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs leading-5 text-ink-mute">
                    This clears page copy, availability overrides, invite copy,
                    and the personal Slack destination. The live status will not
                    change.
                  </p>
          ...
          <div className="space-y-5">
            ...
            >
              <div className="grid gap-4">
                <label className="block" htmlFor={`headline-${page.memberKey}`}>
                  <FieldHeader
                    label="Headline"
            ...
            >
              <div>
                <FieldHeader
                  ...
                  detail={
                    draft.bookableWeekdays === null ? "Inherited" : "Custom"
                  }
                />
                <div
                  ...
                  {WEEKDAYS.map((day) => {
                    ...
                    return (
                      <button
                        ...
                        className={`${FOCUS_RING} rounded-lg border px-2 py-2 text-xs font-medium transition-colors ${active ? "border-ink bg-ink text-paper" : "border-hairline bg-paper text-ink-mute hover:border-hairline-strong hover:text-ink"}`}
                      >
                        {day.short}
                      </button>
                    );
                  ...
                  {webhookConfigured && webhookClearConfirming && (
                    ...
                    >
                      ...
                      <p className="mt-1 text-[11px] leading-5 text-ink-mute">
                        ...
                        team routing only when Slack notifications are enabled.
                      </p>
...
function EditorSection({
  ...
  return (
    <section className="rounded-xl border border-hairline bg-paper-raise p-4 sm:p-5">
      <div className="flex items-start gap-3 border-b border-hairline pb-4">
        <span className="rounded-lg border border-hairline bg-paper p-2 text-ink-mute [&>svg]:h-4 [&>svg]:w-4 [&>svg]:stroke-[1.6]">
          {icon}
        </span>
        <div>
  ...
  return (
    <label className="block" htmlFor={id}>
      ...
      <span className="mt-1.5 block text-[11px] text-ink-faint">
        {custom ? "Overrides the inherited default" : `Inheriting ${defaultText}`}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full border border-hairline bg-paper-raise px-2 py-1 text-[10px] font-medium uppercase tracking-[0.08em] text-ink-faint">
          Non-interactive
        </span>
      </div>

      <div className="mt-3 overflow-hidden rounded-2xl border border-hairline bg-paper-raise shadow-[0_10px_30px_hsl(var(--ink)_/_0.07)]">
              ...
              {visibleDays.length > 0 ? (
                visibleDays.map((day, index) => (
                  <div key={day.value} className="text-center">
                    ...
                    <span
                      className={`mt-1 flex aspect-square items-center justify-center rounded-md text-[9px] ${index === 1 ? "bg-ink text-paper" : "border border-hairline bg-paper-raise text-ink-mute"}`}
                    >
                      {14 + index}
                    </span>
                  </div>
                ...
                <p className="col-span-5 py-3 text-center text-[10px] text-status-warn">
